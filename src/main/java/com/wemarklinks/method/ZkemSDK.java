...
public class ZkemSDK {
	...
	/**
	 * 通过IP地址连接机器，和机器建立一个网络连接 函数原型:VARIANT_BOOL Connect_Net([in] BSTR IPAdd,[in]
	 * long Port1)
	 * 
	 * @param ipAddress
	 *            机器的IP地址
	...
	/**
	 * 通过串口连接机器，即RS232或RS485 函数原型:VARIANT_BOOL Connect_Com([in] long
	 * ComPort,[in] long MachineNumber,[in] long BaudRate)
	 * 
	 * @param comPort
	 *            需要连接机器的PC串口号
	 * @param machineNumber
	...
	public boolean Connect_Com(int comPort, int machineNumber, int baudRate) {
		return zkem.invoke("Connect_Com", new Variant(comPort), new Variant(machineNumber), new Variant(baudRate))
				.getBoolean();
	}

	/**
	 * USB连接考勤机 函数原型:VARIANT_BOOL Connect_USB([in] long MachineNumber)
	 * 
	 * @param machineNumber
	 *            机器号(输入参数)
	 * @return 连接成功返回true,连接失败返回false
	...
	/**
	 * 读取考勤记录到PC的内部缓冲区，同ReadAllGLogData 函数原型:VARIANT_BOOL
	 * ReadGeneralLogData([in] long dwMachineNumber)
	 * 
	 * @param machineNumber
	 *            机器号
	...
	/**
	 * 读取考勤记录到PC内部缓冲区，同ReadAllGLogData 函数原型:VARIANT_BOOL ReadAllGLogData([in]
	 ...
	 * 考勤记录从机器读到PC内部缓冲区中。该函数每执行一次，指向考勤记录的指针向下一条记录 函数原型:VARIANT_BOOL
	 * SSR_GetGeneralLogData([in] LONG dwMachineNumber,[out] BSTR*
	 * dwEnrollNumber, [out] LONG* dwVerifyMode,[out] LONG* dwInOutMode,[out]
	 * LONG* dwYear,[out] LONG* dwMonth, [out] LONG* dwDay,[out] LONG*
	 * dwHour,[out] LONG* dwMinute,[out] LONG* dwSecond,[out]LONG* dwWorkcode)
	 * 
	 * dwMachineNumber:机器号 dwEnrollNumber:指向BSTR型变量的指针，值接收该考勤记录的用户ID号，最大可支持24位
	 ...
	 * dwWorkcode:指向long型变量的指针，其值接收记录的Workcode值 读取成功返回true，读取失败返回false
	 * 
	 * @param machineNumber
	 *            机器号
	 * @return 读取成功返回考勤记录列表
	...
	public List<Map<String, Object>> SSR_GetGeneralLogData(int machineNumber) {
		...
		while (status) {
			...
			status = zkem.invoke("SSR_GetGeneralLogData", new Variant(machineNumber), enrollNumber, verifyMode,
					inoutMode, year, month, day, hour, minute, second, workCode).getBoolean();
	...
	/**
	 * 清除机器内所有考勤记录 函数原型:VARIANT_BOOL ClearGLog([in] dwMachineNumber)
	 * 
	 * @param machineNumber
	 *            机器号
	...
	/**
	 * 读取操作记录到PC的内部缓冲区，同ReadAllSLogData 函数原型:VARIANT_BOOL
	 * ReadSuperLogData([in]long dwMachineNumber)
	 * 
	 * @param machineNumber
	 *            机器号
	...
	/**
	 * 读取操作记录到PC的内部缓冲区，同ReadSuperLogData 函数原型:VARIANT_BOOL
	 * ReadAllSLogData([in]long dwMachineNumber)
	 * 
	 * @param machineNumber
	 *            机器编号
	 * @return 读取缓存成功返回true，读取缓存失败返回false
	 ...
	 * 的是GetSuperLogData2能获取到精确到秒的操作记录时间 函数原型:VARIANT_BOOL
	 * GetSuperLogData([in]long dwMachineNumber,[out]long* dwTMachineNumber,
	 * [out]long* dwSEnrollNumber,[out]long* Params4,[out]long*
	 * Params1,[out]long* Params2, [out]long* dwMainpulation,[out]long*
	 * Params3,[out]long* dwYear,[out]long* dwMonth, [out]long* dwDay,[out]long*
	 * dwHour,[out]long* dwMinute)
	 * 
	 * dwMachineNumber:机器号 dwTMachineNumber:指向long型变量的指针，其值接收操作记录的机器号
	 ...
	 * Params2:指向long型变量的指针，其值含义视dwManipulation不同而不同 dwMainpulation:
	 * 指向long型变量的指针，0开机，1关机，3报警，4进入菜单，5更改设置，6登记指纹，7登记密码
	 * 14创建MF卡，20把卡中数据复制到机器内，22恢复出厂设置，30登记新用户，32胁迫报警，34反潜
	 * Params3:指向long型变量的指针，其值含义视dwManipulation不同而不同 dwYear/dwMonth/
	 * dwDay/dwHour/dwMinute:指向long型变量的指针，其值
	 * 
	 * @param machineNumber
	 *            机器号
	...
	public List<Map<String, Object>> GetSuperLogData(int machineNumber) {
		...
		while (status) {
			...
			status = zkem.invoke("GetSuperLogData", dwMachineNumber, tmachineNumber, senrollNumber, params4, params1,
					params2, mainpulAction, params3, year, month, day, hour, minute).getBoolean();
	 ...
	 * 
	 * 函数原型:VARIANT_BOOL GetAllSLogData([in]long dwMachineNumber,[out]long*
	 * dwTMachineNumber, [out]long* dwSEnrollNumber,[out]long*
	 * Params4,[out]long* Params1,[out]long* Params2, [out]long*
	 * dwMainpulation,[out]long* Params3,[out]long* dwYear,[out]long* dwMonth,
	...
	public List<Map<String, Object>> GetAllSLogData(int machineNumber) {
		...
		while (status) {
			...
			status = zkem.invoke("GetAllSLogData", dwMachineNumber, tmachineNumber, senrollNumber, params4, params1,
					params2, mainpulAction, params3, year, month, day, hour, minute).getBoolean();
	...
	/**
	 * 清除机器内所有操作记录 函数原型:VARIANT_BOOL ClearSLog([in]long dwMachineNumber)
	 * 
	 * @param machineNumber
	 *            机器号
	 * @return 清除成功返回true，清除失败返回false
	 ...
	 * 从内部缓冲区中逐一读取操作记录，使用该函数前，可使用ReadAllSLogData或ReadSuperLogData将操作
	 * 记录从机器读取到PC内部缓冲区中，该函数每执行一次，指向操作记录的指针指向下一条记录，同GetSuperLogData2,不同
	 * 函数原型:VARIANT_BOOL GetSuperLogData2([in]long dwMachineNumber,[out]long*
	 * dwTMachineNumber, [out]long* dwSEnrollNumber,[out]long*
	 * Params4,[out]long* Params1,[out]long* Params2, [out]long*
	 * dwMainpulation,[out]long* Params3,[out]long* dwYear,[out]long* dwMonth,
	...
	public List<Map<String, Object>> GetSuperLogData2(int machineNumber) {
		...
		while (status) {
			...
			status = zkem.invoke("GetSuperLogData2", dwMachineNumber, tmachineNumber, senrollNumber, params4, params1,
					params2, mainpulAction, params3, year, month, day, hour, minute).getBoolean();
	...
	/**
	 * 读取所有用户信息到PC内存中，包括用户编号，密码，姓名，卡号等(指纹模板除外)。该函数执行完成 之后，可叼哟个GetAllUserID获取用户信息
	...
	/**
	 * 设置用户是否可用 函数原型:VARIANT_BOOL SSR_EnableUser([in]LONG
	 * dwMachineNumber,[in]BSTR dwEnrollNumber, [in] VARIANT_BOOL bFlag)
	 * 
	 * 
	 * @param machineNumber
	 *            机器号
	...
	public boolean SSR_EnableUser(int machineNumber, String enrollNumber, boolean flag) {
		return zkem.invoke("SSR_EnableUser", new Variant(machineNumber), new Variant(enrollNumber), new Variant(flag))
				.getBoolean();
	}

	/**
	 * 上传用户验证方式或组验证方式，只有多种验证方式的机器可支持该函数 函数原型:VARIANT_BOOL SetUserInfoEx([in]LONG
	 * dwMachineNumber,[in]LONG dwEnrollNumber, [in]LONG VerifyStyle,[in] BYTE*
	 * Reserved) 彩屏门禁指纹机的值为 :128(FP/PW/RF/FACE), 129(FP), 130(PIN), 131(PW),
	 * 132(RF), 133(FP/PW), 134(FP/RF), 135(PW/RF), 136(PIN&FP), 137(FP&PW),
	 * 138(FP&RF), 139(PW&RF), 140(FP&PW&RF), 141(PIN&FP&PW),
	 * 142(FP&RF/PIN),143(FACE),144(FACE&FP),145(FACE&PW),146(FACE&RF),147(FACE&FP&RF),148(F
	 * ACE&FP&PW).
	 * 
	 * dwMachineNumber:机器号 dwEnrollNumber:用户号 VerifyStyle:验证方式
	 * Reserved:保留参数，暂无意义
	 * 
	 * @param machineNumber
	 *            机器号
	 ...
	 *            用户号
	 * @param verifyStyle
	 *            验证方式,0表示组验证方式，其余参考文档
	 * @return 设置成返回true，设置失败返回false
	 */
	public boolean SetUserInfoEx(int machineNumber, int enrollNumber, int verifyStyle) {
		return zkem.invoke("SetUserInfoEx", new Variant(machineNumber), new Variant(enrollNumber),
				new Variant(verifyStyle), new Variant(null)).getBoolean();
	}

	/**
	 * 获取用户验证方式，只有具有多种验证方式的机器可支持该函数 函数原型:VARIANT_BOOL GetUserInfoEx([in]LONG
	 * dwMachineNumber,[in]LONG dwEnrollNumber, [out]LONG* VerifyStyle,[out]
	 * BYTE* Reserved)
	 * 
	 * @param machineNumber
	 *            机器号
	...
	public Integer GetUserInfoEx(int machineNumber, int enrollNumber) {
		...
		boolean status = zkem.invoke("GetUserInfoEx", new Variant(machineNumber), new Variant(enrollNumber),
	 ...
	 * 执行一次，指向用户信息指针移动到下一条记录，当读完所有的用户信息后，函数返回false 函数原型:VARIANT_BOOL
	 ...
	 * 
	 * dwMachineNumber:机器号 dwEnrollNumber:用户号 Name:用户姓名 Password:用户密码
	 * Privilege:用户权限，3管理员，0普通用户 Enabled:用户启用标志，1为启用，0为禁用
	 * 
	 * @param inMachineNumber
	 *            机器编号
	...
	public List<Map<String, Object>> SSR_GetAllUserInfo(int inMachineNumber) {
		...
		while (status) {
			status = zkem.invoke("SSR_GetAllUserInfo", machineNumber, enrollNumber, name, password, privilege, enable)
					.getBoolean();
	...
	/**
	 * 获取指定用户的信息 函数原型:VARIANT_BOOL SSR_GetUserInfo([in] LONG
	 * dwMachineNumber,[in]BSTR dwEnrollNumber, [out]BSTR* Name,[out]BSTR*
	 * Password,[out]LONG* Privilege,[out]VARIANT_BOOL* Enabled)
	 * 
	 * dwMachineNumber:机器号 dwEnrollNumber:用户号 Name:用户姓名 Password:用户密码
	 * Privilege:用户权限,3管理员，0普通用户 Enabled:用户启用标志，1为启用，0为禁用
	 * 
	...
	public Map<String, Object> SSR_GetUserInfo(int machineNumber, String enrollNumber) {
		...
		boolean status = zkem.invoke("SSR_GetUserInfo", new Variant(machineNumber), new Variant(enrollNumber), name,
				password, privilege, enable).getBoolean();
	...
	/**
	 * 设置指定用户的用户信息，若机内没有该用户，则会创建该用户 函数原型:VARIANT_BOOL SSR_SetUserInfo([in]LONG
	 * dwMachineNumber,[in]BSTR dwEnrollNumber, [in]BSTR Name,[in]BSTR
	 * Password,[in]LONG Privilege,[in]VARIANT_BOOL Enabled)
	 * 
	 * dwMachineNumber:机器号 dwEnrollNumber:用户号 Name:用户姓名 Password:用户密码
	 * Privilege:用户权限，3为管理员，0为普通用户 Enabled:用户启用标志，1为启用，0为禁用
	 * 
	 * @return 设置成功返回true，设置失败返回false
	 */
	public boolean SSR_SetUserInfo(int machineNumber, String enrollNumber, String name, String password, int privilege,
			boolean enable) {
		return zkem.invoke("SSR_SetUserInfo", new Variant(machineNumber), new Variant(enrollNumber), new Variant(name),
				new Variant(password), new Variant(privilege), new Variant(enable)).getBoolean();
	}
	...
	public Map<String, Object> GetUserFaceStr(int machineNumber, String enrollNumber) {
		...
		boolean boolean1 = zkem.invoke("GetUserFace", new Variant(machineNumber), new Variant(enrollNumber), faceIndex,
				tmpData, tmpLenth).getBoolean();
	...
	/**
	 * 删除登记数据 函数原型:VARIANT_BOOL SSR_DeleteEnrollData([in]LONG dwMachineNumber,
	 ...
	 *            用户号
	 * @param backupNumber
	 *            指纹索引，一般范围为0-9，同时会查询该用户是否还有其他指纹和密码，如果没有，则删除该用户。
	...
	public boolean SSR_DeleteEnrollData(int machineNumber, String enrollNumber, int backupNumber) {
		return zkem.invoke("SSR_DeleteEnrollData", new Variant(machineNumber), new Variant(enrollNumber),
				new Variant(backupNumber)).getBoolean();
	}

	/**
	 * 删除登记数据，和SSR_DeleteEnrollData不同的是删除所有指纹数据可用参数13实现，该函数具有更高效率
	 * 函数原型:VARIANT_BOOL SSR_DeleteEnrollDataExt([in]LONG machineNumber,[in]BSTR
	 * enrollNumber,[in]LONG backupNumber)
	 * 
	 * @param machineNumber
	 *            机器号
	 * @param enrollNumber
	...
	public boolean SSR_DeleteEnrollDataExt(int machineNumber, String enrollNumber, int backupNumber) {
		return zkem.invoke("SSR_DeleteEnrollDataExt", new Variant(machineNumber), new Variant(enrollNumber),
				new Variant(backupNumber)).getBoolean();
	}
	...
	/**
	 * 以二进制方式获取用户指纹模板，和SSR_GetUserTmpStr不同的仅是模板格式不同而已 函数原型:VARIANT_BOOL
	 * SSR_GetUserTmp([in]LONG dwMachineNumber,[in]BSTR dwEnrollNumber, [in]LONG
	 * dwFingerIndex,[out]BYTE* TmpData,[out]LONG* TmpLength)
	 * TmpData:该参数返回指纹模板数据 TmpLength:该参数返回指纹模板数据长度
	 * 
	 * @param machineNumber
	 *            机器号
	 * @param enrollNumber
	...
	public Map<String, Object> SSR_GetUserTmp(int machineNumber, int enrollNumber, int fingerIndex) {
		...
		boolean status = zkem.invoke("SSR_GetUserTmp", new Variant(machineNumber), new Variant(enrollNumber),
				new Variant(fingerIndex), tmpdata, tmplength).getBoolean();
	...
	/**
	 * 以字符串方式获取用户指纹模板 函数原型:VARIANT_BOOL SSR_GetUserTmpStr([in]LONG
	 * dwMachineNumber,[in]BSTR dwEnrollNumber, [in]LONG
	 * dwFingerIndex,[out]BYTE* TmpData,[out]LONG* TmpLength)
	 * TmpData:该参数返回指纹模板数据 TmpLength:该参数返回指纹模板数据长度
	 * 
	 * @param machineNumber
	...
	public Map<String, Object> SSR_GetUserTmpStr(int machineNumber, int enrollNumber, int fingerIndex) {
		Variant tmpdata = new Variant("", true);
		Variant tmplength = new Variant(0, true);
		boolean status = zkem.invoke("SSR_GetUserTmpStr", new Variant(machineNumber), new Variant(enrollNumber),
				new Variant(fingerIndex), tmpdata, tmplength).getBoolean();
	...
	/**
	 * 以二进制方式上传用户指纹模板，和SSR_SetUserTmpStr不同的是指纹模板格式不同而已 函数原型:VARIANT_BOOL
	...
	public boolean SSR_SetUserTmp(int machineNumber, String enrollNumber, int fingerIndex, byte tmpData) {
		return zkem.invoke("SSR_SetUserTmp", new Variant(machineNumber), new Variant(enrollNumber),
				new Variant(fingerIndex), new Variant(tmpData)).getBoolean();
	}

	/**
	 * 以字符串方式上传用户指纹模板 函数原型:VARIANT_BOOL SSR_SetUserTmpStr([in]LONG
	 * dwMachineNumber,[in]BSTR dwEnrollNumber, [in]LONG dwFingerIndex,[in]BSTR
	 * TmpData)
	 * 
	 * @param machineNumber
	 *            机器号
	...
	public boolean SSR_SetUserTmpStr(int machineNumber, String enrollNumber, int fingerIndex, String tmpData) {
		return zkem.invoke("SSR_SetUserTmpStr", new Variant(machineNumber), new Variant(enrollNumber),
				new Variant(fingerIndex), new Variant(tmpData)).getBoolean();
	}

	/**
	 * 删除用户指纹模板 函数原型:VARIANT_BOOL SSR_DelUserTmp([in]LONG
	 * dwMachineNumber,[in]BSTR dwEnrollNumber,[in]LONG dwFingerIndex)
	 * 
	 * @param machineNumber
	 *            机器号
	 * @param enrollNumber
	...
	public boolean SSR_DelUserTmp(int machineNumber, String enrollNumber, int fingerIndex) {
		return zkem.invoke("SSR_DelUserTmp", new Variant(machineNumber), new Variant(enrollNumber),
	 ...
	 *            机器号
	 * @param isDeleted
	 *            删除标准，即上传时已存在该用户的指定索引号的指纹是否覆盖原指纹，1为覆盖，0为不覆盖
	 * @param enrollNumber
	 *            用户号
	 * @param fingerIndex
	 *            指纹索引号 一般为0-9
	 ...
	 * @return 设置成功返回true，设置失败返回false
	 */
	public boolean SSR_SetUserTmpExt(int machineNumber, int isDeleted, String enrollNumber, int fingerIndex,
			byte tmpData) {
		return zkem.invoke("SSR_SetUserTmpExt", new Variant(machineNumber), new Variant(isDeleted),
				new Variant(enrollNumber), new Variant(fingerIndex), new Variant(tmpData)).getBoolean();
	}
	...
	public boolean SSR_DelUserTmpExt(int machineNumber, String enrollNumber, int fingerIndex) {
		return zkem.invoke("SSR_DelUserTmpExt", new Variant(machineNumber), new Variant(enrollNumber),
				new Variant(fingerIndex)).getBoolean();
	}

	/**
	 * 以二进制方式上传用户普通指纹模板或者胁迫指纹模板，和SetUserTmpExStr不同的仅是指纹模板格式不同而已
	 * 注意:机器上必须已存在该用户或者将用户信息同时上传，相同用户的相同索引号模板如果已经登记，则覆盖。 注:要求机器固件支持胁迫指纹功能
	 * 
	 * @param machineNumber
	 *            机器号
	 * @param enrollNumber
	 ...
	 *            指纹索引号一般为0-9
	 * @param flag
	 *            标识指纹模板是否有效或为胁迫指纹，0表示指纹模板无效，1表示指纹模板有效，3表示为胁迫指纹
	 * @param tmpData
	 *            指纹模板数据
	 * @return 设置成功返回true，设置失败返回false
	 */
	public boolean SetUserTmpExStr(int machineNumber, String enrollNumber, int fingerIndex, int flag, String tmpData) {
		return zkem.invoke("SetUserTmpExStr", new Variant(machineNumber), new Variant(enrollNumber),
				new Variant(fingerIndex), new Variant(flag), new Variant(tmpData)).getBoolean();
	}
	 ...
	 *            指纹索引号一般为0-9
	 * @param flag
	 *            标识指纹模板是否有效或者是否为胁迫指纹，0表示指纹模板无效，1表示指纹模板有效，3表示胁迫指纹
	 * @param tmpData
	 *            指纹模板数据
	 * @param tmpLength
	 *            指纹模板长度
	 ...
	 */
	@Deprecated // 类型不匹配
	public Map<String, Object> GetUserTmpEx(int machineNumber, String enrollNumber, int fingerIndex, int flag,
			byte tmpData, int tmpLength) {

		Variant v_flag = new Variant(0, true);
		Variant tmpdata = new Variant(0, true);
		Variant tmplength = new Variant(0, true);

		boolean status = zkem.invoke("GetUserTmpEx", new Variant(machineNumber), new Variant(enrollNumber),
				new Variant(fingerIndex), v_flag, tmpdata, tmplength).getBoolean();
	...
	/**
	 * 以字符串方式下载用户普通指纹模板或者胁迫指纹模板 注：要求机器固件支持胁迫指纹功能(固件内部版本号Ver6.60及以上)
	 * 
	 * @param machineNumber
	 *            机器号
	 * @param enrollNumber
	 ...
	 * @return Map<String,Object>格式的用户指纹模板数据
	 */
	public Map<String, Object> GetUserTmpExStr(int machineNumber, String enrollNumber, int fingerIndex) {

		Variant v_flag = new Variant(0, true);
		Variant tmpdata = new Variant("", true);
		Variant tmplength = new Variant(0, true);

		boolean status = zkem.invoke("GetUserTmpExStr", new Variant(machineNumber), new Variant(enrollNumber),
				new Variant(fingerIndex), v_flag, tmpdata, tmplength).getBoolean();
	...
	/**
	 * 根据节假日编号获取机器上的节假日设置 函数原型:VARIANT_BOOL SSR_GetHoliday([in]LONG
	 * dwMachineNumber,[in]LONG HolidayID, [out]LONG* BeginMonth,[out]LONG*
	 * BeginDay,[out]LONG* EndMonth,[out]LONG EndDay,[out]LONG TimeZoneID)
	 * 
	 * dwMachineNumber:机器号 HolidayID:节假日编号
	...
	public Map<String, Object> SSR_GetHoliday(int machineNumber, int holidayID) {
		...
		boolean status = zkem.invoke("SSR_GetHoliday", new Variant(machineNumber), new Variant(holidayID), beginMonth,
				beginDay, endMonth, endDay).getBoolean();
	...
	/**
	 * 设置节假日 函数原型:VARIANT_BOOL SSR_SetHoliday([in]LONG dwMachineNumber,[in]LONG
	 * HolidayID, [in]LONG BeginMonth,[in]LONG BeginDay,[in]LONG
	 * EndMonth,[in]LONG EndDay,[in]LONG TimeZoneID)
	 * 
	 * 
	 * @param machineNumber
	 *            机器号
	 ...
	 * @return 设置成功返回true，设置失败返回false
	 */
	public boolean SSR_SetHoliday(int machineNumber, int holidayID, int beginMonth, int beginDay, int endMonth,
			int endDay, int timeZoneID) {
		return zkem
				.invoke("SSR_SetHoliday", new Variant(machineNumber), new Variant(holidayID), new Variant(beginMonth),
						new Variant(beginDay), new Variant(endMonth), new Variant(endDay), new Variant(timeZoneID))
				.getBoolean();
	}
	...
	/**
	 * 设置是否使用夏时令功能，以及夏时令开始时间和结束时间 函数原型:VARIANT_BOOL SetDaylight([in]LONG
	 * dwMachineNumber,[in]LONG Support [in]BSTR BeginTime,[in]BSTR EndTime)
	 * 
	 * @param machineNumber
	 *            机器号
	 * @param support
	 *            是否使用夏时令功能，1为启用，0为禁用
	 * @param beginTime
	 *            夏时令开始时间，日期格式为 mmdd hh:mm
	 * @param endTime
	 *            夏时令结束时间，日期格式为 mmdd hh:mm
	 * @return 设置成功返回true，设置失败返回false
	 */
	public boolean SetDaylight(int machineNumber, int support, String beginTime, String endTime) {
		return zkem.invoke("SetDaylight", new Variant(machineNumber), new Variant(support), new Variant(beginTime),
				new Variant(endTime)).getBoolean();
	}

	/**
	 * 获取机器的夏时令 函数原型:VARIANT_BOOL GetDaylight([in] dwMachineNumber,[out]LONG*
	 * Support, [out]BSTR* BeginTime,[out]BSTR* EndTime)
	 * 
	 * @param machineNumber
	 *            机器号
	...
	public Map<String, Object> GetDaylight(int machineNumber) {
		...
		boolean status = zkem.invoke("GetDaylight", new Variant(machineNumber), support, beginTime, endTime)
				.getBoolean();
	...
	/**
	 * 计算指定指纹模板长度 函数原型:VARIANT_BOOL GetFPTempLength([in]BYTE*
	 * enrollData,[out]LONG* Len)
	 * 
	 * Len:指纹模板的长度
	 * 
	 * @param enrollData
	...
	/**
	 * 计算指定指纹模板长度 函数原型:VARIANT_BOOL GetFPTempLength([in]BSTR*
	 * enrollData,[out]LONG* Len)
	 * 
	 * Len:指纹模板的长度
	 * 
	...
	/**
	 * 脱机指纹模板转换为BIOKEY指纹模板，和FPTempConvertStr的区别在于格式不同而已 函数原型:VARIANT_BOOL
	 * FPTempConvert([in]BYTE* TmpData1,[out]BYTE* TmpData2,[out]LONG* Size)
	 * TmpData2:返回转换后的BIOKEY指纹模板 Size:返回转换后的BIOKEY指纹模板的大小
	 * 
	 * @param tmpData1
	 *            要转换的脱机指纹模板
	...
	/**
	 * 脱机指纹模板转换为BIOKEY指纹模板，和FPTempConvertStr的区别在于格式不同而已 函数原型:VARIANT_BOOL
	 * FPTempConvertStr([in]BSTR* TmpData1,[out]BSTR* TmpData2,[out]LONG* Size)
	 * TmpData2:返回转换后的BIOKEY指纹模板 Size:返回转换后的BIOKEY指纹模板的大小
	 * 
	 * @param tmpData1
	 *            要转换的脱机指纹模板
	...
	/**
	 * 将BIOKEY指纹模板转换为脱机指纹模板，和FPTempConvertNewStr的区别在与数据格式不同而已 函数原型:VARIANT_BOOL
	 * FPTempConvertNew([in]BYTE* TmpData1,[out]BYTE* TmpData2,[out]LONG* Size)
	 * TmpData2:返回转换后的脱机指纹模板 Size:脱机指纹模板的大小
	 * 
	 * @param tmpData1
	 *            要转换的脱机指纹模板
	...
	/**
	 * 将BIOKEY指纹模板转换为脱机指纹模板 函数原型:VARIANT_BOOL FPTempConvertNewStr([in]BSTR*
	 * TmpData1,[out]BSTR* TmpData2,[out]LONG* Size) TmpData2:返回转换后的脱机指纹模板
	 * Size:脱机指纹模板的大小
	 * 
	 * @param tmpData1
	 *            要转换的脱机指纹模板
	 ...
	 *            机器号
	 * @param dataFlag
	 *            该参数指定需清除的记录类型，范围为1-5，具体含义如下：1考勤记录，2指纹模板数据，3无，4操作记录，5用户信息及指纹模板
	 * @return 清除成功返回true，清除失败返回false
	 */
	 ...
	 *            机器号
	 * @param dataFlag
	 *            需要获取的数据文件类型：1考勤记录数据文件，2指纹模板数据文件，3无，4操作记录数据文件，5用户信息数据文件，6短消息数据文件
	...
	public boolean GetDataFile(int machineNumber, int dataFlag, String fileName) {
		return zkem.invoke("GetDataFile", new Variant(machineNumber), new Variant(dataFlag), new Variant(fileName))
				.getBoolean();
	}

	/**
	 * 发送文件到机器，一般发送到/mnt/mtdblock/下，彩屏机如传的是用户照片或者宣传图片， 需命名为以下格式：图片会自动被转移到相应的目录下
	...
	/**
	 * 获取机器内数据存储状态，例如管理员数，当前用户数等 函数原型:VARIANT_BOOL GetDeviceStatus([in]long
	 * dwMacineNumber,[in]long dwInfo,[out]long* dwValue)
	 * 
	 * @param machineNumber
	 *            机器号
	 * @param status
	 *            需获取的数据，范围1-22，含义如下：1管理员数量，2注册用户数量，3机器内指纹模板数量，4密码数量，5操作记录数
	...
	public Integer GetDeviceStatus(int machineNumber, int status) {
		...
		boolean status_b = zkem.invoke("GetDeviceStatus", new Variant(machineNumber), new Variant(status), value)
				.getBoolean();
	...
	/**
	 * 获取机器相关信息，例如语言，波特率等 函数原型:VARIANT_BOOL GetDeviceInfo([in]long
	 * dwMachineNumber,[in]long dwInfo,[out]long* Value)
	 * 
	 * @param machineNumber
	 *            机器号
	 * @param info
	 *            需获取的信息类型，范围为1-68(注：不能为65)，具体含义如下
	 ...
	 *            9：232/485通讯波特率，0:1200bps,1:2400,2:4800,3:9600,4:19200,5:38400,6:57600,其他
	 *            :115200 10奇偶校验，总是返回0，11停止位，总是返回0。。。。具体参数请查看脱机开发文档。。。。
	 * 
	 * @return 具体返回值与info值设置有关
	 */
	public Integer GetDeviceInfo(int machineNumber, int info) {
		Variant value = new Variant(0, true);
		boolean status = zkem.invoke("GetDeviceInfo", new Variant(machineNumber), new Variant(info), value)
				.getBoolean();
	 ...
	 *            机器号
	 * @param info
	 *            欲设置的信息类型，范围为1-20，其含义参考GetDeviceInfo函数该参数含义
	 * @param value
	 *            要设置的值，请参考info含义
	 * @return 设置成功返回true，设置失败返回false
	 */
	public boolean SetDeviceInfo(int machineNumber, int info, int value) {
		return zkem.invoke("SetDeviceInfo", new Variant(machineNumber), new Variant(info), new Variant(value))
				.getBoolean();
	}
	...
	public boolean SetDeviceTime2(int machineNumber, int year, int month, int day, int hour, int minute, int second) {
		return zkem.invoke("SetDeviceTime2", new Variant(machineNumber), new Variant(year), new Variant(month),
				new Variant(day), new Variant(hour), new Variant(minute), new Variant(second)).getBoolean();
	}
	...
	public Map<String, Object> GetDeviceTime(int machineNumber) {
		...
		boolean status = zkem
				.invoke("GetDeviceTime", new Variant(machineNumber), year, month, day, hour, minute, second)
				.getBoolean();
	 ...
	 *            机器号
	 * @param option
	 *            参数名称：当该参数为字符串"~ZKFPVersion"时，由value描述的返回值为10，
	...
	public String GetSysOption(int machineNumber, String option) {
		Variant value = new Variant("", true);
		boolean status = zkem.invoke("GetSysOption", new Variant(machineNumber), new Variant(option), value)
				.getBoolean();
	...
	public boolean SetSysOption(int machineNumber, String option, String value) {
		return zkem.invoke("SetSysOption", new Variant(machineNumber), new Variant(option), new Variant(value))
				.getBoolean();
	}
	 ...
	 *            需要登记的用户指纹索引号，范围为0-9
	 * @param flag
	 *            标识指纹模板是否有效或者为胁迫指纹，0表示指纹模板无效，1表示指纹模板有效，3表示为胁迫指纹
	 * 
	 * @return 设置成功返回true ，设置失败返回fasle
	 */
	public boolean StartEnrollEx(String userID, int fingerID, int flag) {
		return zkem.invoke("StartEnrollEx", new Variant(userID), new Variant(fingerID), new Variant(flag)).getBoolean();
	}
	 ...
	 * 
	 * @param fullImage
	 *            是否获取整个图像，true为整个图像，false只是指纹部分
	 * @param width
	 *            指定捕获的图像的宽度
	 * @param height
	 *            指定捕获的图像的高度
	 ...
	 *            该参数接收指定的二进制格式的指纹图像
	 * @param imageFile
	 *            该参数指定捕获的指纹图像的保存名(需包含路径)
	 * @return 设置成功返回true，设置失败返回false
	 */
	public boolean CaptureImage(boolean fullImage, int width, int height, byte image, String imageFile) {
		return zkem.invoke("CaptureImage", new Variant(fullImage), new Variant(width), new Variant(height),
				new Variant(image), new Variant(imageFile)).getBoolean();
	}
	 ...
	 *            机器号
	 * @param upateFlag
	 *            存在指纹覆盖标志，即上传用户指纹模板时，如该用户指纹所有已经存在，是否 覆盖致歉的指纹模板，1为强制覆盖，0为不覆盖
	 * 
	 * @return 缓存成返回true，失败返回false
	 */
	public boolean BeginBatchUpdate(int machineNumber, int upateFlag) {
		return zkem.invoke("BeginBatchUpdate", new Variant(machineNumber), new Variant(upateFlag)).getBoolean();
	}
	...
	public boolean SetTZInfo(int machineNumber, int TZIndex, String TZ) {
		return zkem.invoke("SetTZInfo", new Variant(machineNumber), new Variant(TZIndex), new Variant(TZ)).getBoolean();
	}
	...
	public String GetTZInfo(int machineNumber, int TZIndex) {
		...
		boolean b = zkem.invoke("GetTZInfo", new Variant(machineNumber), new Variant(TZIndex), TZ).getBoolean();
		log.info("获取时间段信息: {}", b);
		return TZ.toString();
	}
